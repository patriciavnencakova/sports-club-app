...
def pos(request):
    url = reverse('login')
    link_text = f'Úspešne si sa zaregistroval. Prihlásiť sa môžeš <a href="{url}">sem</a>.'
...
def registration(request):
    if request.method == 'POST':
        ...
        team = account.team
        member = Member(team=team, name=name, surname=surname, birth=birth, email=email, is_active=True)
        account.save()
        member.save()
